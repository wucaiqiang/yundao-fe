...
export default class NewsColumnList extends Base {
  ...
  getDictionary() {
    this
      .dictionary
      .gets("dic_article_column_status")
      .then(res => {
    ...
    const columns = [
      {
        ...
        dataIndex: "url",
        render: (text, record) => {
          const url = (record.platformUrl || 'www.xxx.com') + '/' + record.code + '_news';
        ...
        dataIndex: "statuss",
        render: (text, record) => {
          return filters['dic_article_column_status'] && filters['dic_article_column_status'].filter(item => item.value == record.status)[0].label;
        }
      }, {
        title: "操作人",
        dataIndex: "handlers",
        ...
        render: (text, record) => {
          return Utils.formatDate(record.handlerDate);
        }
      }
    ];
  ...
  render() {
    ...
    // const ProductNewBtn = Permission(     <Button className="btn_add"
    // onClick={()=>this.editColumnModal.show()}>       <img         src={icon_add +
    // "@1x.png"}         srcSet={icon_add + "@2x.png 2x"}         alt=""       />
    //     新增栏目     </Button> );
    return (
      <Page {...this.props}>
        <Breadcrumb className="page-breadcrumb">
          <Breadcrumb.Item>内容配置</Breadcrumb.Item>
          <Breadcrumb.Item>文章栏目配置</Breadcrumb.Item>
        </Breadcrumb>
        <Layout.Content>
          <div className="page-content">
            <GridManager
              gridWrapClassName="grid-panel auto-width-grid"
              url="/cms/article/column/get_page"
              onSelect={selectData => {
