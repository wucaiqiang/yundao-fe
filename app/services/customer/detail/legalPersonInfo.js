...
export default class CustomerDetailBaseInfo extends Base {
  ...
  render() {
    ...
    if (info && info.provinceCode) {
      let province = constCitys.filter(item => item.value == info.provinceCode);

      if (province && province.length > 0) {
        provinceAndCity.push(province[0].label);
        if (info.cityCode) {
          let city = province[0].children.filter(
            item => item.value == info.cityCode
          );
          provinceAndCity.push(city && city[0] && city[0].label);
        }
      }
    }
        ...
        <div className={style.content}>
          {!canRead ? (
            ...
            <Form>
              ...
              <FormItem label="出生日期" {...formItemLayout}>
                {info.birthday ? info.birthday.format("YYYY-MM-DD") : null}
              </FormItem>
              <FormItem label="职务" {...formItemLayout}>
                {info.position}
              </FormItem>
              ...
              <FormItem label="是否联系人" {...formItemLayout}>
                {info.isContact ? "是" : "否"}
              </FormItem>
            </Form>
          )}
