...
import style from "./importCustomerForm.scss";
...
class EditCustomerForm extends Base {
  ...
  render() {
    ...
    const tip = file
      ? `<span>尝试上传 ${file.total} 条，成功 ${file.successCount} 条${file.failureCount > 0
          ? `，失败<span class='red'> ${file.failureCount} </span>条，点击<a href=${file.filePath} target='_blank'> 这里 </a>查看错误信息`
          : ""}</span>`
      : null;
    ...
    return (
      <div className={style.body}>
        <div className={style.case}>
          <p className={style.header}>第一步,请按照数据模板的格式准备要导入的数据</p>
