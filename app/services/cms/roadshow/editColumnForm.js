...
class EditRecommendForm extends Base {
  ...
  getDictionary() {
    this.dictionary.gets("dic_roadshow_column_status").then(res => {
  ...
  get_platform = () => {
    return this.platform.get_platform_roadshow().then(res => {
      this.setState({
        platforms: res.result
      });
      this.formUtils.setFieldsValue({
        platformId: res.result[0].id.toString()
      });
    });
  };
  handleChange = value => {
  ...
  render() {
    ...
    if (platforms && platformId) {
      platformUrl =
        (platforms.filter(item => item.id == platformId)[0] &&
          platforms.filter(item => item.id == platformId)[0].url) ||
        "www.xxx.com";
    }
    return (
      <Form
      ...
      >
        ...
        <FormItem
          label="所属平台"
          {...formItemLayout}
          ref={ref => (this.formItem = ReactDom.findDOMNode(ref))}
        >
          {!this.props.isEdit
            ? data.platformName
        ...
        <FormItem label="状态" {...formItemLayout}>
          {!this.props.isEdit
            ? filters["dic_roadshow_column_status"] &&
              filters["dic_roadshow_column_status"].filter(
            ...
            : this.formUtils.getFieldDecorator("status", {
                ...
                <RadioGroup>
                  {filters["dic_roadshow_column_status"] &&
                    filters["dic_roadshow_column_status"].map(item => {
                      return (
                        <Radio value={item.value.toString()}>
                          {item.label}
                        </Radio>
                      );
                    })}
