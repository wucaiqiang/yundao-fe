...
export default class AddDeclarationModal extends Base {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  handleSubmit = e => {
    ...
    this.formUtils.validateFields(errors => {
      if (!errors) {
        ...
        if (formData.declarationModel == 5) {
          request = this.declaration.declaration_patch;
        }
        // const request =
        //   this.state.formData.from == "product.center"
        //     ? this.declaration.center_add
        //     : this.declaration.add;
              ...
              if (res.success) {
                ...
                const modal =Modal.confirm({
                  ...
                  onOk: () => {
                    ...
                    //   `/declaration/detail/${declarationId}`
                    // );
                  }
                });
              }
  ...
  renderFooterBtn() {
    let { declarationModel } = this.state;
    let btns = [];
    if ([3, 4, 5].indexOf(declarationModel) > -1) {
      btns = [
        <Button
          key="cancel"
          onClick={() => {
  ...
  callback = () => {
    const declarationModel = this.formUtils.getFieldValue("declarationModel");
    this.setState({
      declarationModel
    });
    console.log("declarationModel", declarationModel);
  };
  render() {
