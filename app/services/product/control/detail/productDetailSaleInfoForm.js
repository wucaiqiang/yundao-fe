...
class BaseInfoForm extends Base {
    ...
    renderList() {
        ...
        const fields = data.productDto.data.productSaleFieldDtos.map(this.formatFieldType)
        ...
        const renderFormItem =(field,index)=>{
            const isScale = this
                            .formUtils
                            .getFieldValue('isScale') !=undefined  ?this
                            .formUtils
                            .getFieldValue('isScale'): initIsScale

            return field.name == 'productScale' && isScale == 0?null:<DynamicFormItem
                                key={index}
                                field={field}
...
class ProductBaseInfoForm extends Base {
        ...
        this
            ...
            .validateFields((errors) => {
                ...
                if (!errors) {
                    ...
                    // if (values.saleDate) {
                    //     //处理时间
                    //     if(values.saleDate[0]){
                    //         values.saleDate=[values.saleDate[0].format("YYYY-MM-DD"),values.saleDate[1].format("YYYY-MM-DD")].join(',')
                    //     }
                    // }
    ...
    render() {
        ...
        return (
            ...
            <BaseInfoForm formUtils={this.formUtils} data={data}/>
        )
    }
}
//
